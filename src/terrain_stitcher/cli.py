...
def addCreateBoundsGeneratorArgs(subparser):
    ...
    parserGenerate.add_argument(
        "-r", "--range", type=str, help="Range in miles of coverage"
    )
...
def addPrepOrthoImages(subparser):
    ...
    parserGenerate.add_argument("-f", "--scaleFactor", help="Scale amount")
    parserGenerate.add_argument(
...
def moveShapeFile(height_info_file, src, outputDir):
...
def main():
    ...
    if args.command == "create-bounds":
        ...
        final_info_file = os.path.join(args.output, "height_info.json")
        main_prep_ortho(args.input, args.output, float(args.scaleFactor), args.shapeFile)
        main_prep_elevation(
            args.elevationDataDir, args.output, args.elevationDataDir, args.shapeFile
        )
        moveShapeFile(final_info_file, args.shapeFile, args.output)
    else:
